          ...
          >
            ...
            {message.role === "user" && message.sttMs != null && (
              <div className="mt-1.5 pt-1.5 border-t border-text/5 text-[10px] text-text/30 font-mono">
                <span>
                  {t("conversation.metrics.sttMs", { ms: message.sttMs })}
                </span>
              </div>
            )}
                ...
                {message.tokensPerSec != null && (
                  <span>
                    {t("conversation.metrics.tokensPerSec", {
                      tps: message.tokensPerSec.toFixed(1),
                    })}
                  </span>
                )}
                {message.totalMs != null && (
                  <span>
                    {t("conversation.metrics.totalMs", { ms: message.totalMs })}
                  </span>
                )}
                {message.ttsMs != null && (
                  <span>
                    {t("conversation.metrics.ttsMs", { ms: message.ttsMs })}
                  </span>
                )}
              </div>
            )}
