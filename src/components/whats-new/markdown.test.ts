...
test("the shipped release notes parse to non-empty blocks", () => {
  ...
  const link = first(linked, "paragraph").children.find(
    (n) => n.type === "link",
  );
  ...
  assert.ok(
    captioned && captioned.type === "paragraph",
    "0.9.0: the screenshot",
  );
...
test("the parser accepts exactly the documented subset", () => {
...
test("the three deliberate divergences from a full CommonMark parser", () => {
