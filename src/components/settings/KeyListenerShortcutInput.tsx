...
interface KeyListenerShortcutInputProps {
...
> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopKeyListenerRecording().catch(console.error);
  ...
  // Set up event listener for key-listener events
  useEffect(() => {
    if (!isRecording) return;
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<KeyListenerEvent>(
        "key-listener-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopKeyListenerRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      const result = await commands.startKeyListenerRecording(shortcutId);
