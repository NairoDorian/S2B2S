    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (insideStructOrEnum) {
        ...
        if (line.startsWith('}') || line.endsWith('}')) {
          ...
          const isSpectaType = joined.includes('Type') && (joined.includes('derive') || joined.includes('specta'));
          if (isSpectaType) {
            ...
            for (const lineOfStruct of structLines) {
              ...
              if (fieldMatch) {
                ...
                if (containsBigint) {
                  ...
                  const hasOverride = prevLineIdx >= 0 && structLines[prevLineIdx].includes('specta(type =');
                  if (!hasOverride) {
                    console.log(`File: ${path.relative(srcDir, file)} | Struct: ${structName}`);
