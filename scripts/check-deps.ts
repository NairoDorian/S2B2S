...
// ─────────────────────────────────────────────────────────────────────────
try {
  const rustcV = execSync("rustc --version", { encoding: "utf8" })
    .trim()
    .split(" ")[1];
  addResult({
    name: "Rust (rustc)",
  ...
  const cargoV = execSync("cargo --version", { encoding: "utf8" })
    .trim()
    .split(" ")[1];
  addResult({
    name: "Cargo",
...
if (pythonCmd) {
  try {
    const pyVersion = execSync(`${pythonCmd} --version`, {
      encoding: "utf8",
      timeout: 5000,
    }).trim();
    addResult({
      name: "Python",
      current: pyVersion.replace("Python ", ""),
      status: "info",
      source: "venv/system",
    });
  } catch {
...
// ─────────────────────────────────────────────────────────────────────────
try {
  const tauriV = execSync(
    "bun run tauri -- --version 2>&1 || cargo tauri --version 2>&1",
    {
      encoding: "utf8",
      timeout: 15_000,
      cwd: projectRoot,
...
if (errors.length > 0) {
  ...
  for (const r of errors) {
    console.log(
      `  ${RED}✗${RESET} ${r.name.padEnd(28)} ${RED}${r.current}${RESET}`,
    );
  }
  console.log("");
}
...
if (outdated.length)
  summary.push(`${YELLOW}${outdated.length} outdated${RESET}`);
