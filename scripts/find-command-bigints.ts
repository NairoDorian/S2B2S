...
function analyze() {
  ...
  for (const file of files) {
    ...
    if (!content.includes('tauri::command') && !content.includes('specta::specta')) continue;

    const lines = content.split('\n');
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i].trim();
      if (line.includes('fn ') && (lines[i - 1]?.includes('tauri::command') || lines[i - 1]?.includes('specta') || lines[i - 2]?.includes('tauri::command'))) {
        ...
        for (const bt of bigintTypes) {
          ...
          if (regex.test(signature)) {
            console.log(`File: ${path.relative(srcDir, file)} | Command: ${line}`);
