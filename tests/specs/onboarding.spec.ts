...
test.describe("Onboarding Flow", () => {
  test("runs through the complete onboarding flow successfully", async ({ page }) => {
    ...
    const installBtn = page.locator("button:has-text('Install Speech Runtime')");
    ...
    const modelSelectionTitle = page.locator("text=To get started, choose a transcription model");
    await expect(modelSelectionTitle).toBeVisible({ timeout: 10000 });

    // 8. Trigger model download
    const downloadBtn = page.locator("role=button[name*='Parakeet V3']").first();
