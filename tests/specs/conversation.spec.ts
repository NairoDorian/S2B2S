...
test.describe("Conversation Tab", () => {
  test("sends message and renders LLM streamed reply with barge-in support", async ({
    page,
  }) => {
        ...
        brain_ask: (args: any) => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                (window as any).__mockEmit(
                  "brain:token",
                  " there! I am your AI assistant.",
                );
                setTimeout(() => {
                  (window as any).__mockEmit("brain:done", {
    ...
    const stopBtn = page
      .locator(
        "button:has-text('Stop'), button:has-text('Abort'), .cancel-button, [title*='Stop'], [title*='Abort']",
      )
      .first();
    if (await stopBtn.isVisible()) {
      await stopBtn.click();
      const abortCalled = await page.evaluate(
        () => (window as any).__abortCalled,
      );
