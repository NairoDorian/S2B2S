...
await model.transcribe(pcm, {
  family: { kind: "whisper", initialPrompt: "…" },
});
...
or an _active stream_ — across all of its sessions. The binding enforces this:
...
directory at _build_ time — there is nothing special to wire through.
...
if (!fs.existsSync(`${dir}/${lib}`)) {
  throw new Error(
    `native library not found in ${dir}; the platform package is not bundled`,
  );
}
