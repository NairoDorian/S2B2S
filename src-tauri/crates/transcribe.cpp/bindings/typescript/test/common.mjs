...
export const STREAMING_MODEL =
  process.env.TRANSCRIBE_SMOKE_STREAMING_MODEL || "";
export const PARAKEET_STREAM_MODEL =
  process.env.TRANSCRIBE_SMOKE_PARAKEET_STREAM_MODEL || "";
export const PARAKEET_BUFFERED_MODEL =
  process.env.TRANSCRIBE_SMOKE_PARAKEET_BUFFERED_MODEL || "";
...
export const AUDIO =
  process.env.TRANSCRIBE_SMOKE_AUDIO ||
  path.resolve(HERE, "../../../samples/jfk.wav");
...
export async function feedChunks(stream, pcm, chunk = 16000) {
  for (let i = 0; i < pcm.length; i += chunk)
    await stream.feed(pcm.subarray(i, i + chunk));
}
