...
modelTest(
  "whisper run extension applies; accepts() discriminates",
  MODEL,
  async () => {
...
modelTest(
  "run() rejects a stream-slot family with InvalidArgument",
  MODEL,
  async () => {
    const m = await TranscribeModel.load(MODEL);
    try {
...
modelTest(
  "parakeet cache-aware stream extension",
  PARAKEET_STREAM_MODEL,
  async () => {
...
modelTest(
  "parakeet buffered stream extension",
  PARAKEET_BUFFERED_MODEL,
  async () => {
