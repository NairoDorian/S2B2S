...
modelTest("streaming commits text and finalizes", STREAMING_MODEL, async () => {
  ...
  try {
    ...
    assert.equal(
      stream.lastStatus,
      null,
      "a healthy finished stream has no failure status",
    );
    const t = stream.text;
    assert.ok(
      (t.committed + t.full).trim().length > 0,
...
modelTest(
  "stream reads reject while a feed is in flight",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    try {
      ...
      assert.throws(
        () => stream.text,
        /in flight/i,
        "reading .text mid-feed must throw",
      );
...
modelTest(
  "an active stream holds a model-wide compute lease",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    try {
...
modelTest(
  "reset() and dispose() are safe during an un-awaited feed",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    try {
      ...
      assert.equal(
        stream.state,
        "idle",
        "reset() returns the stream to idle synchronously",
      );
...
modelTest(
  "disposing a session with an active stream frees the lease",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    try {
      ...
      await assert.rejects(
        () => stream.feed(jfk().subarray(0, 16000)),
        /disposed/i,
      );
...
modelTest(
  "disposing a model with an active stream is crash-safe",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    const s = m.createSession();
    ...
    assert.ok(
      true,
      "no native crash after model dispose with an active stream",
    );
  },
);

modelTest(
  "a stream queued before another's reset is refused, not overlapped",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    try {
...
modelTest(
  "a stream queued before another's dispose is refused, not overlapped",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    try {
...
modelTest(
  "a stream begin queued before its own dispose is rejected, not leaked",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    try {
...
modelTest(
  "a stale stream wrapper cannot reset a newer same-session stream",
  STREAMING_MODEL,
  async () => {
    const m = await TranscribeModel.load(STREAMING_MODEL);
    try {
