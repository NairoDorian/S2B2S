...
if (!libDir || !tuple || !version || !outRoot) {
  console.error(
    "usage: --lib-dir <dir> --tuple <tuple> --version <x.y.z> --out <dir>",
  );
  process.exit(2);
}
...
if (!spec) {
  console.error(
    `unknown tuple ${tuple}; known: ${Object.keys(TUPLES).join(", ")}`,
  );
  process.exit(2);
}
...
for (const f of fs.readdirSync(libDir)) {
  if (f.includes(libExt) || f.endsWith(".so") || /\.so\.\d/.test(f)) {
    ...
    fs.cpSync(path.join(libDir, f), path.join(pkgDir, f), {
      recursive: true,
      dereference: true,
    });
    files.push(f);
...
if (!files.includes(spec.lib)) {
  console.error(
    `expected ${spec.lib} in ${libDir}; found: ${files.join(", ") || "(none)"}`,
  );
  process.exit(1);
}
...
if (fs.existsSync(path.join(libDir, "transcribe-link.json"))) {
  fs.cpSync(
    path.join(libDir, "transcribe-link.json"),
    path.join(pkgDir, "transcribe-link.json"),
  );
...
const headerHash =
  arg("header-hash") ||
  fs
    .readFileSync(path.join(REPO, "include", "transcribe.abihash"), "utf8")
    .trim();
...
fs.writeFileSync(
  path.join(pkgDir, "contract.json"),
...
console.log(
  `  os=${spec.os} cpu=${spec.cpu}${spec.libc ? ` libc=${spec.libc}` : ""}`,
);
console.log(
  `  backends=${contract.backends.join(",")} header_hash=${contract.header_hash}`,
);
