...
console.log(
  `  install prefix: ${prefix}${selfContained ? " (self-contained)" : ""}`,
);

const flags = ["-DTRANSCRIBE_BUILD_SHARED=ON"];
if (process.platform === "darwin")
  flags.push("-DGGML_METAL=ON", "-DGGML_METAL_EMBED_LIBRARY=ON");
if (selfContained) flags.push("-DBUILD_SHARED_LIBS=OFF"); // ggml static -> one libtranscribe
if (process.env.TRANSCRIBE_CMAKE_FLAGS)
  flags.push(...process.env.TRANSCRIBE_CMAKE_FLAGS.split(" ").filter(Boolean));
