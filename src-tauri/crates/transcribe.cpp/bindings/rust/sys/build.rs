...
fn main() {
    ...
    if shared { active_features.push("shared"); }
    if dynamic_backends { active_features.push("dynamic-backends"); }
    if feature("METAL") { active_features.push("metal"); }
    if feature("VULKAN") { active_features.push("vulkan"); }
    if feature("CUDA") { active_features.push("cuda"); }
    if feature("OPENMP") { active_features.push("openmp"); }

    let cache_key = compute_cache_key(&target_os, &target_arch, &target_env, &active_features);
    let cache_dir = get_cache_root().join(&cache_key);

    // Check for explicit prebuilt or persistent cache hit
    if !force_rebuild {
    ...
    println!("cargo:warning=transcribe-cpp-sys: [CACHE MISS] Compiling transcribe-cpp via CMake...");
    ...
    // so a user -D wins on the first configure.
...
/// Root directory for persistent out-of-tree transcribe.cpp artifacts.
...
/// Compute a unique key for the target platform and enabled features.
