...
- **Timestamps move it ~0.2pp.** `segment` → 2.63%, `none` → 2.81%. This is
  ...
  `none` to match OpenAI/Open ASR Leaderboard. `segment` is _better_ here
...
`--collar-ms` is the _total_ width of a collar centered on every reference
boundary, pyannote's parameterization: `--collar-ms 250` excludes 125 ms on
each side. NIST md-eval's `-c` is _per side_ — to reproduce a published
