...
- **Voxtral Realtime 2602** (`voxtral_realtime`): a _streaming_ audio-LLM
...
- Text decoder (`LlamaForCausalLM`): standard Ministral/Mistral-Small.
  ...
  **NEOX RoPE** (theta 1e8, q/k pre-permuted by the converter),
  `attention_bias=false`, **untied lm_head**, `max_position_embeddings` 131072.
...
- Pattern: **audio-llm**, _streaming_ (additive fusion).
...
- **Realtime** (`VoxtralRealtimeFeatureExtractor`, no
  ...
  (`global_log_mel_max=1.5`, _not_ the per-utterance max) so per-frame
...
- **Sinusoidal positional embedding** `audio_tower.embed_positions.weight`
  ...
  checkpoint — _not_ synthesized here, unlike the HF convert script).
