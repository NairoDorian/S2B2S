...
- **Batch (offline) design.** `run_batch` batches the _decoder_ (batched KV
  cache + `block_prefill_batched` + `run_batched_step_loop`) — the dominant cost
  of transcription. The _encoder_ runs per-utterance (`encode_one`) in both the
  ...
  _fused_ batched-encoder dump `<name>.b{i}` against the single-shot, as
