...
`cl.exe` will _not_ be on your global `PATH`, and that's expected — CMake's
...
`vulkan-1.lib`); the Vulkan _runtime_ loader (`vulkan-1.dll`) already ships
...
> **Performance note — measure _warm_, not the first run.** A single
...
> the first run and makes a one-shot timing look _slower_ than CPU. Warm
