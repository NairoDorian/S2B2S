...
_before_ running; some accept any length but lose accuracy past the window they
...
reserve — the same shape as the upfront gate, but the gate uses the _exact_
...
computes the prefill size _before_ running the encoder and rejects an
...
Moonshine is the honest edge case in this bucket: its cap is on _output_
...
is a _model-level_ capability, queried before any session exists, so it always
...
status of _that step_, not a verdict on the whole transcript. So when a
...
- The upfront gate and `max_audio_ms` share a shape for decoder-context-bound
  ...
  where `ceiling` is the GGUF max. The gate uses the _exact_ per-call prompt, so
