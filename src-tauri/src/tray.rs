...
/// On Windows, reads the *system* theme (taskbar) from the registry so that
/// the tray icon contrasts correctly even when the app theme differs from
/// the taskbar theme (e.g., dark taskbar + light apps).
pub fn get_current_theme(app: &AppHandle) -> AppTheme {
    if cfg!(target_os = "linux") {
        ...
        // Fallback to window theme if registry read fails
...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/s2b2s.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("S2B2S v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("S2B2S v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "s2b2s-1.wav".to_string(),
            timestamp: 0,
            saved: false,
