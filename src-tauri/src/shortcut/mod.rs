...
//! - `key_listener`: Uses the s2b2s-keys library for more control
//!
...
// Note: Commands are accessed via shortcut::key_listener:: in lib.rs
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::KeyListener | KeyboardImplementation::HandyKeys => {
            if let Err(e) = key_listener::init_shortcuts(app) {
                ...
                // Update settings to persist the fallback so we don't retry KeyListener on next launch
...
pub fn change_keyboard_implementation_setting(
    ...
    // Initialize new implementation if needed (KeyListener needs state)
    if (new_impl == KeyboardImplementation::KeyListener
        || new_impl == KeyboardImplementation::HandyKeys)
        && initialize_key_listener_with_rollback(&app)?
    {
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "key_listener" => KeyboardImplementation::KeyListener,
        other => {
            warn!(
...
/// Initialize KeyListener if not already initialized, with rollback on failure
fn initialize_key_listener_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<key_listener::KeyListenerState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = key_listener::init_shortcuts(app) {
        error!("Failed to initialize KeyListener: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        ...
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize KeyListener: {}. Reverted to Tauri.",
            e
        ));
    }
...
#[tauri::command]
#[specta::specta]
pub fn change_hold_threshold_ms_setting(app: AppHandle, ms: u32) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.hold_threshold_ms = ms as u64;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_result_ready_audio_feedback_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.result_ready_audio_feedback = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
// ============================================================================
// Language Models & Post-Process Actions CRUD
// ============================================================================
...
pub fn change_selected_microphone_auto_switch_enabled_setting(
        ...
        app.try_state::<std::sync::Arc<crate::managers::audio::AudioRecordingManager>>()
    {
        if let Err(err) = rm.update_selected_device() {
            log::warn!(
                "Failed to refresh microphone stream after auto-switch toggle: {}",
                err
            );
        }
    }
...
pub fn change_selected_microphone_name_pattern_setting(
        ...
        app.try_state::<std::sync::Arc<crate::managers::audio::AudioRecordingManager>>()
    {
        if let Err(err) = rm.update_selected_device() {
            log::warn!(
                "Failed to refresh microphone stream after mask change: {}",
                err
            );
        }
    }
