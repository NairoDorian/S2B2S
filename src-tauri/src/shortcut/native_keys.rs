//! Native keyboard shortcut implementation
//!
//! This module provides an alternative to Tauri's global-shortcut plugin,
//! built on the third-party `handy-keys` crate for more control over keyboard
//! events than the plugin exposes. The crate's name is not ours and is not
//! renameable; everything this module owns is named after what it does.
//!
//! ## Architecture
//!
...
/// Convert the crate’s `Modifiers` to a list of strings
...
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    ...
    // `shortcut::should_register_binding` so every backend agrees.
    for (id, default_binding) in default_bindings {
        ...
        if !crate::shortcut::should_register_binding(&user_settings, &binding) {
            continue;
        }
