...
fn init_gtk_layer_shell(overlay_window: &tauri::webview::WebviewWindow) -> bool {
    if env_flag_enabled("S2B2S_NO_GTK_LAYER_SHELL") {
        debug!("Skipping GTK layer shell init (S2B2S_NO_GTK_LAYER_SHELL is enabled)");
        return false;
    }
...
pub fn emit_levels(app_handle: &AppHandle, levels: &[f32]) {
    ...
    // Throttle to ~30 FPS (issue #1279 memory leak mitigation) for the recording overlay,
    // while still fanning out to the brain overlay every callback.
    ...
    if now.saturating_sub(last) >= EMIT_THROTTLE_MS {
        ...
        // Target only the overlay window (single eval_script per callback).
