...
pub fn paste(text: String, app_handle: AppHandle) -> Result<(), String> {
    ...
    // Apply smart decapitalization if enabled
    let text = if settings.text_replacement_decapitalize_after_edit_key_enabled {
        crate::text_replacement_decapitalize::maybe_decapitalize_next_chunk_standard(&text)
    } else {
        text
    };
...
pub fn capture_selection_text(app_handle: &AppHandle) -> Result<String, String> {
    ...
    let previous_text = if advanced_backup.is_none() {
    ...
    if captured == sentinel || captured.trim().is_empty() {
        ...
        if let Some(backup) = advanced_backup {
            let _ = win_clipboard::restore_all_formats(backup);
        } else {
