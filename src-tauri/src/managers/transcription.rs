    ...
    fn create_engine(
        ...
        let engine = match model_info.engine_type {
            EngineType::TranscribeCpp => {
                ...
                let bound_device = model
                    .device()
                    .map(|d| transcribe_device_label(&d))
                    .unwrap_or_else(|_| "unknown".to_string());
                ...
                info!(
                    "Loaded whisper model '{}' (requested backend {:?}, device {}; bound {:?}, device {}; supports_streaming={}, supports_translate={}, supports_language_detect={})",
                    model_id,
                    backend,
                    requested_device,
                    bound_backend,
