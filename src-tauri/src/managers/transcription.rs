...
pub struct TranscriptionManager {
    ...
    /// These are loaded alongside the primary engine for simultaneous
    /// multi-model transcription and kept resident in RAM/VRAM between calls.
...
impl TranscriptionManager {
    ...
    /// Create a `LoadedEngine` from model info and model path.
    /// Shared between primary model loading and multi-STT extra model loading.
    /// Uses the persisted accelerator settings (no device_index override).
    fn create_engine(
        &self,
        model_id: &str,
        ...
        let engine = match model_info.engine_type {
            EngineType::TranscribeCpp => {
                ...
                let session = model.session().map_err(|e| {
                    anyhow::anyhow!("Failed to create session for {}: {}", model_id, e)
                })?;
                // Reconcile capabilities from the loaded GGUF metadata
                let caps = session.model().capabilities();
                self.model_manager.set_runtime_capabilities(
                    model_id,
                    caps.supports_streaming,
                ...
                info!(
                    "Loaded whisper model '{}' (backend {:?}, gpu_device {}, supports_streaming={}, supports_translate={}, supports_language_detect={})",
                    model_id, backend, gpu_device, caps.supports_streaming, caps.supports_translate, caps.supports_language_detect
                );
                LoadedEngine::TranscribeCpp(session)
            }
            EngineType::Parakeet => {
    ...
    /// Load an extra (multi-STT) model into the resident engine cache.
    /// Subsequent calls to [`transcribe_with_extra`] will reuse this engine
    /// instead of re-loading from disk, keeping the model in RAM/VRAM between calls.
    ...
    /// Unload a single extra model engine, freeing its RAM/VRAM.
    ...
    /// Get the list of extra model IDs currently loaded in the cache.
    pub fn get_extra_loaded_models(&self) -> Vec<String> {
        let extra = self.extra_engines.lock().unwrap();
        extra.keys().cloned().collect()
    }

    /// Check if a specific extra model is loaded in the cache.
    pub fn is_extra_model_loaded(&self, model_id: &str) -> bool {
        let extra = self.extra_engines.lock().unwrap();
        extra.contains_key(model_id)
    }

    /// Unload all extra (multi-STT) engines. Called on shutdown and when
    /// multi-STT settings change to free VRAM.
    pub fn unload_all_extra_models(&self) {
        ...
        for model_id in ids {
            if let Err(e) = self.unload_extra_model(&model_id) {
                warn!("Failed to unload extra model '{}': {}", model_id, e);
            }
        }
    }

    /// Transcribe audio with a cached extra model engine.
    /// The engine is temporarily removed from the cache for exclusive use,
    /// transcribed, and then returned to the cache so it stays in RAM/VRAM.
    ///
    /// Per-model language and translate overrides are read from
    /// `multi_stt_language_model_2/3` and `multi_stt_translate_model_2/3`
    /// in the app settings.
    pub fn transcribe_with_extra(&self, model_id: &str, audio: Vec<f32>) -> Result<String> {
        ...
        let (transcription, engine_to_return) = match result {
            ...
            Err(panic_payload) => {
                ...
                error!(
                    "Multi-STT extra model '{}' engine panicked: {}. Model has been unloaded.",
                    model_id, panic_msg
                );
                (
                    Err(anyhow::anyhow!(
        ...
        // Return engine to the cache if it survived and hasn't been unloaded
    ...
    fn run_stream_worker(&self, rx: mpsc::Receiver<StreamCmd>, worker_id: u64) {
        ...
        let stream_started = 'stream: {
            ...
            let stream_ext = {
                ...
                let latency_kind = model_info.and_then(|info| info.native_streaming_latency_kind);
                let preset = settings
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        if std::env::var("S2B2S_FORCE_TRANSCRIPTION_FAILURE").is_ok() {
            return Err(anyhow::anyhow!(
                "Simulated transcription failure (S2B2S_FORCE_TRANSCRIPTION_FAILURE)"
            ));
        }
        ...
        let validated_language = match self.model_manager.get_model_info(&active_model) {
            ...
            None => resolved_language.clone(),
        };

        if validated_language != settings.selected_language {
            debug!(
                "Language intent '{}' (resolved to '{}') resolved to '{}' for model '{}'",
                settings.selected_language, resolved_language, validated_language, active_model
            );
        }
...
/// Used by `transcribe_with_extra` for multi-STT. Settings may carry per-model
/// language/translate overrides applied by the caller.
fn transcribe_with_engine(
    ...
    match engine {
        ...
        LoadedEngine::UnifiedParakeet(server) => {
            let text = server
                .transcribe(audio)
                .map_err(|e| anyhow::anyhow!("Unified Parakeet transcription failed: {}", e))?;
            Ok(text)
        }
    }
}
...
impl Drop for TranscriptionManager {
    fn drop(&mut self) {
        ...
        // Unload any extra (multi-STT) engines that are still resident
        self.unload_all_extra_models();
    }
}
