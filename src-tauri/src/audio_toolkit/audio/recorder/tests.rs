...
fn speech_clock_earshot_16ms_frames_measure_time_accurately() {
    ...
    let mut earshot_clock = super::SpeechClock::new(2, 500, 16, Arc::clone(&published));
    assert_eq!(earshot_clock.frame_ms(), 16);

    feed(&mut earshot_clock, true, 2);
    assert_eq!(earshot_clock.snapshot().speech_ms, 32);

    feed(&mut earshot_clock, true, 48);
    assert_eq!(earshot_clock.snapshot().speech_ms, 50 * 16);
    assert_eq!(published.load(std::sync::atomic::Ordering::Relaxed), 800);
}
...
fn speech_clock_publishes_its_total_for_readers_off_thread() {
    let published = Arc::new(std::sync::atomic::AtomicU64::new(0));
    let mut clock = super::SpeechClock::new(2, 500, FRAME_MS, Arc::clone(&published));
    ...
    assert!(published.load(std::sync::atomic::Ordering::Relaxed) > 0);

    let frozen = published.load(std::sync::atomic::Ordering::Relaxed);
