...
- **New `settings.models.hub` i18n namespace** (`ModelsSettings.tsx`): Added a `hideTitle` prop so the page can be embedded as a tab without a duplicate header.
- **Unified Model Hub page** (`ModelHubSettings.tsx`): Consolidates STT, Brain (Gemma 4), audio.cpp TTS, and llama-server runtimes into a single tabbed Models page (`src/components/settings/models/ModelHubSettings.tsx` + `src/stores/hubStore.ts`). A cross-collection "Active Downloads" bar aggregates progress via the new typed `model-hub-download-progress` / `model-hub-notification` events. STT models still render through the existing catalog manager (sha256 + resumable downloads).
- **Hub backend** (`model_hub/`): New typed commands `hub_get_active_downloads`, `hub_cancel_download`, `hub_delete_model` and typed events `ModelHubDownloadProgress` / `ModelHubNotification`, registered with `tauri-specta`.
- **Shared resumable download transport** (`model_hub/transport.rs`): Centralized HTTP downloader with range-resume, stall watchdog, sha256 verification, and cancellation — now used by the Brain Gemma 4 downloads, audio.cpp packages, and llama-server runtime downloads.
- **Brain model downloads gain sha256 pinning, resume, cancel & delete** (`brain/llama_manager.rs`): Downloads now verify LFS sha256 from the Hugging Face tree API, resume partial files, emit typed hub progress, and support cancellation/deletion plus a `get_downloaded_brain_models` listing.
- **`Tabs` UI primitive** (`src/components/ui/Tabs.tsx`): Accessible tab-bar (keyboard navigation, ARIA) for the Hub.
- **Multi-STT Brain-mode selector** (`settings.rs`, `multi_stt.rs`, `actions.rs`, `continuous_voice.rs`, `MultiSttSettings.tsx`, `MultiSttConversationControls.tsx`): The retired `multi_stt_gemma4_enabled` / `multi_stt_merge_include_audio` toggles are replaced by a single `multi_stt_brain_mode` setting (`text_only` | `separate_asr` | `audio_in_merge`) with a one-time migration for existing stores. Extra models and the Gemma 4 ASR pass now transcribe in parallel with the primary model (`spawn_parallel` / `join_spawned` / `spawn_gemma4`); the Brain path skips the separate merge LLM round-trip and lets the Brain fuse transcripts + audio itself. History entries now carry the per-model transcript block, the merge provider's reasoning (if any), and the Brain's final reply.
- **Separate reasoning capture in chat completions** (`llm_client.rs`, `brain/client.rs`, `brain/manager.rs`): `reasoning_content` is now captured independently of the answer in both the SSE Brain client and the non-streaming post-process client (`ChatCompletionOutcome`, `send_chat_completion_with_reasoning*`); `BrainManager::last_reasoning()` exposes the latest turn's chain-of-thought for history/debugging without polluting the spoken/pasted answer.
...
- **Removed `multimodal_audio_enabled` / `multimodal_image_enabled`** (`settings.rs`, `bindings.ts`): The per-modality legacy fields are gone entirely — serde ignores unknown keys, so old stores still load. All runtime logic uses `llama_mmproj_enabled` directly.
...
- **Duplicate llama-server binary downloads / churn** (`llama_server/manager.rs`): `download_server` now early-returns when the requested `backend-tag` binary is already installed, dedupes in-flight downloads, and `remove_server` refuses to delete the directory of the currently running server (preventing the re-download loop seen in dev logs). Downloads now stream progress + emit the typed hub event.
- **TypeScript bindings export silently failed (BigInt)** (`lib.rs`): The `specta_builder` export step emitted `Attempted to export ''` warnings for u64 `size_bytes`/`size_mb` fields; affected DTOs now use `#[specta(type = u32)]` overrides. The `export_bindings` test passes cleanly and `src/bindings.ts` regenerates without warnings.
- **Hotkey init errors for unassigned shortcuts** (`shortcut/key_listener.rs`): Empty/unassigned hotkeys and the legacy orphan `pause` F6 binding that double-registered are now skipped quietly during init instead of erroring.
- **Dev builds logged a false "could not fetch a valid release JSON" updater error on every startup** (`UpdateChecker.tsx`): Automatic updater checks are now skipped in dev builds (manual checks still work), removing startup noise.
- **Noisy "Dropping DeviceSink" log spam** (`tts/player.rs`): `MixerDeviceSink` now calls `log_on_drop(false)`, silencing rodio's drop-time warning when playback ends or the server stops.
- **Python TTS engines ignored portable/app-data model directory** (`tts/local_tts_server.rs`): `resolve_local_models_dir` now prefers the portable `<app_data>/models/TTS/` directory when a Tauri app handle is available, fixing model discovery for portable installs; the previous CWD-relative fallback chain is retained for dev.
...
- **Qwen3 TTS chunk streaming removed — whole-audio buffered synthesis restored** (`tts/mod.rs`, `tts/backends/qwen3.rs`, `tts/player.rs`, `tts/manager.rs`, `qwen3_server.py`): The chunk-level streaming mode (HTTP JSON-header + raw PCM stream, `synthesize_streaming`, `player.append_pcm` / `stream_begin` / `stream_end`) never produced fluid audio on this machine and was stripped end-to-end. `TtsBackend` only exposes buffered `synthesize()`, the player's streaming Cmds/state are gone, and the server's `do_POST` only serves whole WAVs. TTS now synthesizes the entire utterance in one request, then plays it — the same path the greeting always used (which never chopped). The sentence/fragment pipeline (`tts_shorten_first_chunk`, 3-fragment pattern) still gives per-fragment time-to-first-audio for long text.
- **Qwen3 generation batching 2 → 12** (`tts/backends/qwen3.rs`, `qwen3_server.py`): Benchmarking on this machine (RTX 4070 Laptop, torch backend, 3-iter medians with discarded warmup) showed the small-chunk premise was wrong here: chunk 2 pays ~85ms fixed per-chunk Python/CUDA overhead → RTF 0.72x (barely real-time — any GPU contention from llama-server/STT starved the sink into gaps), while chunk 12 (≈1s of audio) measures RTF 0.43x (~2.4x real-time headroom) with ~460ms first audio — the throughput plateau. `chunk_size` is now purely an internal generator batching knob for the buffered path (the faster-qwen3-tts `non_streaming_mode=True` API); the `--chunk-size` server arg and `DEFAULT_CHUNK_SIZE` both use 12, making the buffered path ~2x faster than at chunk 2. See `docs/qwen3-chopped-audio-postmortem.md` for the full investigation.
- **Qwen3 request timeout killed long synthesis** (`tts/backends/qwen3.rs`): The HTTP deadline was `(8s + chars*50ms).clamp(15s, 300s)` — a short utterance got a 15s ceiling that expired mid-synthesis when generation ran slower than real-time, aborting the request, unloading the server (~20s model reload), and cascading "error sending request" failures for the remaining fragments. The deadline is now speech-duration-aware (~12 chars/s, 6x real-time-factor slack) with a 60s floor and 30min cap.
- **Debug tab now has a single aggregated log console** (`LogViewer.tsx`, `DebugSettings.tsx`, `LiveLogViewer.tsx` deleted): The Debug tab previously rendered two consoles with different content — a live `log://log` event stream (only logs emitted while the panel was open) and a separate file-backed log viewer. Both are now merged into one `LogViewer` that seeds from the on-disk log file (`get_recent_logs`) and live-appends the backend event stream in real time. Live-streamed lines are reconciled with their on-disk counterpart on the next file poll (count-aware merge, so repeated identical lines survive), pause buffers incoming events and resume flushes + re-syncs, and the severity filter, search, line-limit, auto-refresh, copy, and clear controls all operate on the single unified stream. i18n: `debug.logViewer.*` keys extended (title, severity labels, confirm/toast texts, live status); the stale `settings.debug.liveLogs.*` keys removed from all locales.
- **Multi-STT hotkey conflict** (`settings.rs`): `multi_stt_transcribe` defaulted to the same binding as `transcribe_with_post_process` (ctrl+shift+space), so one always failed to register. New unique default (`ctrl+alt+shift+space` / macOS `cmd+option+shift+space`) plus a one-time schema-v2 migration that resets the multi-STT binding when it still collides.
- **Replaced multi-STT models unloaded** (`MultiSttConversationControls.tsx`): swapping the 2nd/3rd conversation model now unloads the previous one from VRAM (unless it is still configured in the other slot).
- **Python env: safe venv recreation + "Install All" parity** (`python_env.rs`, `commands/python_env.rs`): `create_venv` no longer deletes the existing venv in place — it renames it aside, creates the fresh venv, and only removes the backup on success (a failed run restores the old venv, so installed packages can never be lost). Backend installs now recreate a hollow/invalid venv (missing `pyvenv.cfg`). `install_all_backends` (the Install All button) mirrors `scripts/setup_venv_uv.ps1` exactly: all backends in canonical order (kokoro-tts → pocket → kitten → sherpa → torch → audio/ML deps → qwen-tts → piper last) with CUDA runtime at the very end, one run. Kokoro now installs `kokoro-tts` and is detected via `kokoro_tts` import.
- **Multi-STT conversation controls always visible** (`ConversationView.tsx`): the conversation Multi-STT row (enable, 2nd/3rd model slots, Gemma 4, audio-in-merge) is no longer hidden when the Brain is disabled.
- **Multi-STT 2nd/3rd model dropdown names** (`MultiSttSettings.tsx`): Model slots 2 and 3 filtered their own selected model out of the dropdown options, so the selected name never rendered (placeholder shown instead), and slot 3 wrongly excluded slot 2's model. Options now keep their current selection, exclude the primary model and the other slot's model, and resolve display names against the full catalog.
- **llama.cpp mmproj upgrade across use cases** (`llama_manager.rs`, `multi_stt.rs`, `manager.rs`): Consumers now declare whether they need multimodal input (`ensure_server_running_with`). If the local server is already running text-only and a multimodal consumer arrives (Gemma 4 STT, audio-inclusive merge, audio/image conversation), the server is restarted automatically with mmproj loaded; text-only consumers reuse any running server as-is.
...
- **audio.cpp Native C++ GGUF Engine Integration & Model Manager** (`audiocpp_server/`, `tts/backends/audiocpp.rs`, `AudioCppModelManager.tsx`, `SpeechSettings.tsx`, `scripts/compile-audiocpp.ps1`, `scripts/sync-all-repos.ps1`): Integrated `0xShug0/audio.cpp` native C++ GGML/GGUF audio runtime into S2B2S. Added a complete in-app GGUF model management interface with search, task filters, quantization variant picker (Original, Q8_0, F16, etc.), and streaming background downloader from Hugging Face. Added multi-task session handling (`clon`, `vc`, `tts`, `asr`, `gen`) with native demo voice presets (`demo_1_man`..`demo_4_woman`) for Chatterbox and Supertonic 3.
- **Brain Model Reasoning / Thinking Toggle** (`settings.rs`, `llama_manager.rs`, `client.rs`, `BrainSettings.tsx`): Added a dedicated `reasoning_enabled` setting and UI toggle for the local Llama.cpp / Gemma 4 Brain. Dynamically configures `--reasoning on` (for deep multi-step reasoning) or `--reasoning off` (for lowest latency in conversational voice mode) when launching `llama-server`. Reasoning tokens and `<thought>` blocks are captured and filtered so internal monologue is never read aloud over TTS. Full i18n support across 23 languages.

- **Qwen3-TTS Language Selection** (`qwen3_server.py`, `settings.rs`, `qwen3.rs`, `SpeechSettings.tsx`): New `tts.qwen3.language` setting (default `"auto"` = per-utterance auto-detect) surfaced as a Language dropdown in Speech settings. The server now accepts a `language` request param on every synthesis (validated case-insensitively against the loaded model's table, unknown values fall back to `"auto"`) and threads it through all four paths: faster-qwen3-tts buffered custom voice, voice cloning, voice design, the official qwen-tts CPU fallback, and the GGML/qwentts.cpp backend (`lang`). A new `GET /languages` endpoint reports the supported set (mirrors `talker_config.codec_language_id` minus the auto-applied dialects), exposed to the frontend via the new `qwen3_get_languages` command (`Qwen3Backend::list_languages` with the live-server response preferred over the static checkpoint table). Dialects (Sichuan/Beijing) still auto-apply on Eric/Dylan when the language is chinese/auto. Also fixed a latent bug: changing the Qwen3 model no longer wipes other `tts.qwen3` fields (state patch now merges instead of replacing).
- **Automatic Microphone Selection (M3.6)** (`settings.rs`, `managers/audio.rs`, `AutomaticMicrophoneMask.tsx`, `settingsStore.ts`): New `selected_microphone_auto_switch_enabled` + `selected_microphone_name_pattern` settings with a UI toggle and pattern input (supports `*`/`?` wildcards, case-insensitive; plain substrings match by containment). When enabled and a device name matches the pattern, that device is used for recording automatically; otherwise the app falls back to the clamshell probe, then the manually selected microphone. `update_selected_device` is now guarded by `RecordingState::Idle`, so changing the mic can never kill an in-progress recording.
- **Pause Media While Recording (M3.6)** (`settings.rs`, `managers/audio.rs`, `PauseMediaWhileRecording.tsx`): New `pause_media_while_recording` toggle. When enabled, the app pauses playing music/videos when recording starts and resumes them when recording ends (Windows: GSMT WinRT sessions; Linux: `playerctl`; macOS: osascript Music/Spotify/QuickTime Player). Backend note: windows-rs 0.62 removed the blocking `IAsyncOperation::get()`, so WinRT async calls are blocked via `futures_executor::block_on(operation.into_future())` (`futures-executor` added to Windows deps — already in the tree).
...
- **Python Environment Manager** (`python_env.rs`, `commands/python_env.rs`, `python-env/PythonEnvSettings.tsx`, `portable.rs`): New settings panel plus Tauri commands to detect/install `uv`, create or recreate the shared Python 3.12 venv, and install individual or all TTS/STT backend packages (CPU or CUDA) with streaming progress events. Venv directory resolution is shared with `portable.rs` (`get_venv_dir`) so all platform paths agree.
- **Live Brain server status in the footer** (`llama_manager.rs`, `commands/brain.rs`, `BrainSelector.tsx`): Added typed `llama-server-status` event and `get_brain_server_status` command reporting running/loading/stopped state, loaded model, mmproj, compute backend and port. The footer Brain indicator now reflects the local server's true state no matter which feature started it (conversation, post-processing, or multi-STT merge/Gemma 4 STT), including an mmproj badge.
- **Multi-STT transcripts fused by the Brain** (`multi_stt.rs`, `actions.rs`, `manager.rs`): `transcribe_parallel` now returns the individual per-model transcripts alongside the merged consensus. In conversation mode these are listed in the model-facing user message, so Gemma 4 receives the 1-3 raw transcripts plus the raw audio and performs the final fusion itself — the conversation history still stores only the clean consensus text.
- **Multi-STT controls in Conversation view** (`MultiSttConversationControls.tsx`, `commands/transcription.rs`, `commands/brain.rs`, `lib.rs`): The conversation view now has inline Multi-STT controls — enable toggle, 2nd/3rd model dropdowns, Gemma 4 STT toggle, and audio-in-merge toggle. Enabling preloads the extra models into RAM/VRAM (`preload_multi_stt_models`, also run at startup when active) and warms the llama.cpp server with mmproj when multimodal fusion is on (`warm_brain_server`); disabling unloads the extras to free VRAM.
- **Per-quant model downloads + latency info** (`commands/models.rs`, `model.rs`, `ModelQuantPicker.tsx`, `ModelsSettings.tsx`): The model download menu now lists every quantization the handy-computer catalog offers (Q4_K_M … F32) with sizes as one-click chips — downloading any quant works through the standard HF/mirror/verify pipeline — and shows a configurable-latency note for streaming families (Nemotron, Parakeet Unified). Verified against all 69 `handy-computer` Hugging Face repos.
- **Latency preset auto-download** (`NativeStreamingLatencySelector.tsx`): Changing a streaming-latency preset on a model that isn't downloaded yet now downloads it automatically (with a toast), so the new preset can take effect.
