...
**`improvement-plan.md`** — Deep-dive analysis of the streaming STT, streaming TTS, and converted-model subsystems. Key findings: (1) the self-exported Parakeet-Unified streaming model is correctly built and wired into main, verified byte-for-byte against sherpa-onnx's upstream exporter; (2) "streaming" is actually replay — audio chunks are fed after recording stops, not during live capture, so no real-time partials or latency benefit; (3) the converted model has no download source (local-only gate); (4) two parallel STT paths (hand-rolled ONNX vs sherpa-onnx) carry maintenance load and fragile DSP; (5) TTS "streaming" is fragment pipelining, not sample-level streaming. Prioritized roadmap (P0–P2): publish the model, implement true live-mic streaming, converge STT paths onto sherpa-onnx, make Nemotron 3.5 (80 ms, 40 langs) the default streaming model, and move to persistent transport for per-chunk overhead.
