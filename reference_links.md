...
> **No rankings here.** This document only *describes* each project; head-to-head comparisons are intentionally left out.
...
> S2B2S forks **Handy**, and several entries below are *also* Handy forks — useful to see how others extended the same base.
...
### mrtozner/vox
**Local-first voice-AI *framework* in Rust** wiring the whole loop: Mic → VAD (Silero) → STT (Whisper / Distil-Whisper / Sherpa-ONNX) → Speaker ID → your code → TTS (Kokoro / Piper / Qwen3 / Pocket / Chatterbox) → speaker. Adds experimental real-time speaker diarization (voice embeddings, auto-enrollment, persistent DB) and "Live Talk" barge-in voice chat. Referenced in S2B2S notes for its **voice-over-WebSocket** protocol design.
...
> A different paradigm from the cascaded loop: a single model that listens *and* speaks at once (no explicit speaker turns), preserving prosody/emotion. Heavy today, but the direction worth watching for a future S2B2S "natural conversation" mode.
...
> The core of the *standalone Android app* goal: running the "brain" locally on the phone. Mix of libraries (drop into your own app) and reference apps.
...
> On-device read-aloud on the phone — the TTS half of the standalone Android app. Several register as the *system* TTS engine, so any app's `TextToSpeech` API can use them.

### woheller69/ttsEngine — SherpaTTS
**An Android *system* TTS engine based on Next-gen Kaldi (sherpa-onnx),** using **Piper** or **Coqui** voices. Because it registers as a system TTS service, it works in any app that uses the standard Android TTS API. On F-Droid. (Note: the README warns it may stop working on certified Android devices after Google's 2026/2027 developer-identity requirement.)
...
*Compiled 19 June 2026. Dates reflect public repo activity at fetch time and will drift — re-check before relying on "latest release." Descriptions paraphrased from project READMEs/About pages; see each linked repo for authoritative details and licenses.*
